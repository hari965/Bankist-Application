...
const displaySummary = function (acc) {
    ...
    labelSumOut.textContent = `${Math.abs(outgoing)} Rs`

    const interestRate = acc.movements.filter(mov => mov > 0).map(deposit => deposit * acc.interestRate).filter((int, i, arr) => int >= 1).reduce((acc, interests) => acc + interests, 0);
    labelSumInterest.textContent = `${interestRate} Rs`
}
